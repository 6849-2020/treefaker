...
describe("buildFaces", function() {
  it("on optimal 4-leaf star, constructs two faces with correct orientation", function() {
    ...
    expect(creases).to.eql([
      CreaseType.ActiveHull,
      CreaseType.ActiveHull,
      CreaseType.ActiveHull,
      CreaseType.ActiveHull
    ]);
  ...
  it("on cleaned 10-leaf star tree, constructs the right number of faces and sets new hull edge pointers", function() {
    ...
    const crease104 = g.getEdge(
      g.nodes.get("4") as CreasesNode,
      g.nodes.get("10") as CreasesNode
    ) as Crease;
  ...
  it("on small example from paper, makes two faces and one inactive hull edge", function() {
    ...
    const crease12 = g.getEdge(
      g.nodes.get("1") as CreasesNode,
      g.nodes.get("2") as CreasesNode
    ) as Crease;
    expect(crease12.creaseType).to.equal(CreaseType.ActiveHull);
    const crease15 = g.getEdge(
      g.nodes.get("1") as CreasesNode,
      g.nodes.get("5") as CreasesNode
    ) as Crease;
